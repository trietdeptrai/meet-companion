export function sampleFrameMarkers(timeline) {
  const duration = Math.max(Number(timeline?.duration_sec) || 10, 1);
...
export function runVisualQa({ video, timeline, componentGraph, preflight }) {
  ...
  if (Number(video?.durationSeconds) > 10 || Number(timeline?.duration_sec) > 10) {
    issues.push({
      severity: "high",
      type: "duration_limit_exceeded",
      description: "Local demo videos must stay within the 10 second cap.",
      suggested_fix: "Compress timeline shot durations.",
    });
  }
...
export function createRepairPlan({ qaReport, timeline }) {
  ...
  if (qaReport.issues.some((issue) => issue.type === "duration_limit_exceeded")) {
    changes.push({
      ...
      change: "Timeline remains capped to the local 10 second duration.",
    });
  }
